        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:13,overflow:'hidden'}}>
          ...
          {filtered.map((o, idx) => (
            <div key={o.id}>
              ...
              onMouseLeave={e=>e.currentTarget.style.background=expanded.has(o.id)?'rgba(245,166,35,.04)':'transparent'}
              >
                {/* Sifariş no */}
                ...
                <div style={{textAlign:'center'}} onClick={e=>e.stopPropagation()}>
                  {o.tracking.length > 0 ? (
                    <a href={o.tracking[0].url||'#'} target="_blank" rel="noreferrer" style={{
                      background:'rgba(56,189,248,.12)', border:'1px solid rgba(56,189,248,.25)',
                      color:'#38bdf8', fontSize:11, fontWeight:700, padding:'4px 8px',
                      borderRadius:6, fontFamily:'var(--sans)', textDecoration:'none',
                      display:'inline-block'
                    }}>
                      📦 {o.tracking[0].number?.slice(-6)}
                    </a>
                  ) : (
              ...
              {expanded.has(o.id) && (
                <div style={{
                  ...
                  {/* Məhsullar */}
                  <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom: o.tracking.length?14:0}}>
                    {(o.items||[]).map((item,i)=>{
                      const isRefunded = item.refunded_qty > 0
                      const isFulfilled = item.fulfilled_qty >= item.total_qty
                      const isPending = item.pending_qty > 0

                      return (
                        <div key={i} style={{
                          display:'flex',alignItems:'center',gap:10,
                          padding:'8px 12px',borderRadius:9,
                          background: isRefunded && item.refunded_qty>=item.total_qty
                            ? 'rgba(248,113,113,.08)'
                            : isFulfilled ? 'rgba(74,222,128,.06)'
                            : 'var(--surf2)',
                          border: isRefunded && item.refunded_qty>=item.total_qty
                            ? '1px solid rgba(248,113,113,.2)'
                            : isFulfilled ? '1px solid rgba(74,222,128,.15)'
                            : '1px solid transparent',
                          opacity: isRefunded && item.refunded_qty>=item.total_qty ? .7 : 1,
                        }}>
                          {/* Status ikonu */}
                          <span style={{fontSize:16,flexShrink:0}}>
                            {isRefunded && item.refunded_qty>=item.total_qty ? '↩' :
                             isFulfilled ? '✅' : '⏳'}
                          </span>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{
                              ...
                              textDecoration: isRefunded&&item.refunded_qty>=item.total_qty ? 'line-through' : 'none',
                              color: isRefunded&&item.refunded_qty>=item.total_qty ? 'var(--muted)' : 'var(--text)',
                          ...
                          <div style={{display:'flex',gap:6,alignItems:'center',flexShrink:0}}>
                            {item.fulfilled_qty > 0 && (
                              <span style={{fontSize:11,padding:'2px 8px',borderRadius:10,
                                ...
                                border:'1px solid rgba(74,222,128,.2)',fontWeight:600}}>
                                ✓ {item.fulfilled_qty}
                              </span>
                            )}
                            {item.pending_qty > 0 && (
                              <span style={{fontSize:11,padding:'2px 8px',borderRadius:10,
                                ...
                                border:'1px solid rgba(245,166,35,.2)',fontWeight:600}}>
                                ⏳ {item.pending_qty}
                              </span>
                            )}
                            {item.refunded_qty > 0 && (
                  ...
                  {o.tracking.length > 0 && (
                    <div style={{display:'flex',flexDirection:'column',gap:6}}>
                      <div style={{fontSize:11,fontWeight:700,color:'var(--muted)',
                        textTransform:'uppercase',letterSpacing:.5,marginBottom:4}}>İzləmə</div>
                      {o.tracking.map((t,i)=>(
                        <div key={i} style={{display:'flex',alignItems:'center',gap:10,
                          padding:'8px 12px',borderRadius:9,background:'rgba(56,189,248,.06)',
